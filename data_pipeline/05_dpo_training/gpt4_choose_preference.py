"""
GPT-4가 페르소나 기준으로 variant A/B 중 chosen/rejected 결정
"""
...
class GPT4Chooser:
    ...
    def choose_preference(self, prompt: str, variant_a: str, variant_b: str, persona: dict):
        """
        GPT-4로 variant A와 B 중 chosen/rejected 결정
...
"""
        ...
        try:
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4-turbo",
                messages=[
                    {"role": "system", "content": "You are an expert recipe evaluator."},
    ...
    def process_persona_variants(self, input_file: Path, output_file: Path):
        """
        페르소나별 variants를 GPT-4로 평가하고 chosen/rejected 결정
        """
        # Variants 로드
        variants = []
        ...
        for sample in tqdm(variants, desc="GPT-4 Choosing"):
            ...
            if chosen and rejected:
                ...
                if evaluation.get('recommendation') == 'use_pair':
                    ...
                    rejected_pairs.append({
                        "reason": "gpt4_recommendation_reject",
                        "sample": sample,
                        "evaluation": evaluation
                    })
            else:
...
def main():
    parser = argparse.ArgumentParser(description="GPT-4 chooses chosen/rejected from variants")
